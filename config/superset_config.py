...
BASE_DIR = os.path.join(os.environ['SUPERSET_HOME'], 'superset')
...
class CeleryConfig(object):
    BROKER_URL = ('redis://%s:%s/0' % (REDIS_HOST, REDIS_PORT), 'sqla+sqlite:///celerydb.sqlite')[bool(not REDIS_HOST)]
    CELERY_IMPORTS = (
        'superset.sql_lab',
        'superset.tasks',
    )
    CELERY_RESULT_BACKEND = ('redis://%s:%s/0' % (REDIS_HOST, REDIS_PORT), 'sqla+sqlite:///celerydb.sqlite')[bool(not REDIS_HOST)]
